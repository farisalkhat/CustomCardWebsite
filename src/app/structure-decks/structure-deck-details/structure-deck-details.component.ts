...
@Component({
  ...
  templateUrl: './structure-deck-details.component.html',
  styleUrls: ['./structure-deck-details.component.css']
})
export class StructureDeckDetailsComponent implements OnInit {
...
addPackToCollection(){
    ...
    binderInfo['cards'] = this.decklist;
...
purchase(){
    if(this.currency>=this.deckInfo['cost']){

        let text = "Are you sure you want to purchase this structure deck?";
        if (confirm(text) == true) {
            this._authService.subtractCurrency(this.deckInfo['cost'])
...
confirmDelete(){
  if(this._authService.adminRole()){
    ...
    edit(){
        if(this._authService.adminRole() && this.id == this.deckInfo['creatorid']){
            this._router.navigate(['/structure-deck-editor/'+String(this.deckid)]);
        }
    }
